...
class TrainTransform(object):
    def __init__(self, h, w, random_erase=False):
        self.h = h
        self.w = w
    ...
    def __call__(self, img):
        ...
        for attempt in range(100):
            ...
            if w < img.size()[2] and h < img.size()[1]:
                ...
                if img.size()[0] == 3:
                    img[:, x1:x1 + h, y1:y1 + w] = torch.from_numpy(np.random.random_sample(size=(3, h, w)))
                else:
                    img[0, x1:x1 + h, y1:y1 + w] = torch.from_numpy(np.random.random_sample(size=(1, h, w)))
                return img
